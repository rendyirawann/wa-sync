...
/// Kirim email HTML generik. Mengembalikan true bila terkirim.
pub async fn send_html(to: &str, subject: &str, html: String) -> bool {
    if !configured() {
        return false;
    }
    ...
    let Ok(from_mbox) = format!("{from_name} <{from_addr}>").parse() else { return false };
    let Ok(to_mbox) = to.parse() else { return false };
    let email = match Message::builder().from(from_mbox).to(to_mbox).subject(subject).header(ContentType::TEXT_HTML).body(html) {
        Ok(m) => m,
        Err(e) => { tracing::error!("gagal menyusun email: {e}"); return false; }
    };
    ...
    let transport = match builder {
        ...
        Err(e) => { tracing::error!("gagal inisialisasi SMTP: {e}"); return false; }
    };
    match transport.send(email).await {
        Ok(_) => { tracing::info!("email '{subject}' terkirim ke {to}"); true }
        Err(e) => { tracing::error!("gagal kirim email ke {to}: {e}"); false }
    }
}
